...
import { getMovieCredits, getMovieDetails, getMovieProviders, getSimilarMovies } from '../apis';
...
export default function Movie() {
  ...
  const fetchprovidersDetails = () => {
    setLoading(true);
    getMovieProviders(movieId).then((res) => {
            ...
            <div className='d-flex'>
              ...
              <div className='col-9 text-start pt-3'>
                <div className='movie-tv-heading  ls-2 text-light'>
                  {movieDetails.title}
                  <span className='text-muted small mx-2'>({movieDetails.release_date.split('-')[0]})</span>
                </div>
                <div className='p-0 m-0 small d-flex'>
                  {moment(movieDetails.first_air_date || movieDetails.release_date, 'YYYY-MM-DD').format('MM/DD/YYYY')}
                  <ul>
